...
def percent_probability_validator(value):
    if 1 > value > 100:
        raise ValidationError(f'Ты мне мозги не еби, я спрашиваю СКОЛЬКО? Какие нахуй {value} процентов, дядя?')
...
class FunnyAction(models.Model):
    ...
    word_position = models.CharField(
        ...
        choices=WORD_POSITION_CHOICES
    )

    trigger_word = models.TextField(
    ...
    answer_probability = models.IntegerField(
        validators=[percent_probability_validator],
        verbose_name='Answer probability'
    )
