...
from bot.models import SearchRequest, Message, User, Status
...
class Paginator:
    ...
    def _save_search_request(self):
        self.search_request = SearchRequest.objects.create(
            user=self.user,
            search_text=self.message_text
        )

    def _get_list_of_messages(self, total_amount: int, start_position: int = 0, end_position: int = 25) -> str:
        messages_list = f'Найдено резульатов: {total_amount}\n\n'
        russian_message_types = {'photo': 'Картинка', 'video': 'Видео'}
        for i, message in enumerate(self.messages[start_position:end_position], start_position + 1):
            if i < 10:
                spaces = '  '
            else:
                spaces = ' '
    ...
    def _get_all_messages(self):
        self.messages = Message.objects.filter(text_on_image__icontains=self.message_text).order_by('created_at')
    ...
    def find_messages(self) -> dict:
        ...
        reply_markup = Markup(self.user).next_pages_buttons(self.search_request, pages_count)
        list_of_messages = self._get_list_of_messages(len(self.messages))
        return {'found': True, 'reply_markup': reply_markup, 'message_text': list_of_messages}

    def turn_over_page(self, page: int):
        self._get_all_messages()
        pages_count = self._get_pages_count()
        start_position, end_position = self._get_positions(page)
        reply_markup = Markup(self.user).all_paginator_buttons(self.search_request, pages_count, page)
        list_of_messages = self._get_list_of_messages(len(self.messages), start_position, end_position)
        return {'found': True, 'reply_markup': reply_markup, 'message_text': list_of_messages}
